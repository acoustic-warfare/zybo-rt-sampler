        ...
        for (int n = 0; n < n_arrays; n++)
        {
            for (int y = 0; y < ROWS; y++)
            {
                ...
                if ((y % 2) == 0)
                {
                    ...
                    {
                        rb->data[step + N_SAMPLES * s] = (float)((double)(message->stream[msg_offset + row + x]) / NORM_FACTOR_SOUND);
                        s++;
                    }
                }
                else
                {
                    for (int x = 0; x < COLUMNS; x++)
                    {
                        rb->data[step + N_SAMPLES * s] = (float)((double)(message->stream[msg_offset + row + COLUMNS - x]) / NORM_FACTOR_SOUND);
                        s++;
                    }
                }
            }
